...
let sawPolishingPhase = false;
let sawWarningFixAttempt = false;
...
const req = http.request(
  ...
  (res) => {
    ...
    res.on('data', (chunk) => {
      ...
      while ((idx = buf.indexOf('\n\n')) !== -1) {
        ...
        if (evt.type === 'phase') {
          ...
          if (evt.phase === 'polishing') sawPolishingPhase = true;
        }

        if (evt.type === 'validation_report') {
          const attempt = String(evt.attempt || 'initial');
          console.log('VALIDATION attempt=' + attempt + ' passed=' + evt.passed + ' warnings=' + ((evt.warnings || []).length));
          if (attempt === 'warning-fix-1') sawWarningFixAttempt = true;
        }

        if (evt.type === 'verification_report') {
          const attempt = String(evt.attempt || 'initial');
          console.log('VERIFICATION attempt=' + attempt + ' passed=' + evt.passed + ' warnings=' + ((evt.warnings || []).length));
          if (attempt === 'warning-fix-1') sawWarningFixAttempt = true;
        }

        if (evt.type === 'warning') {
          console.log('WARNING=' + String(evt.message || ''));
        }
    ...
    res.on('end', () => {
      ...
      console.log('SUMMARY polishingPhase=' + sawPolishingPhase + ' warningFixAttempt=' + sawWarningFixAttempt + ' complete=' + sawComplete + ' error=' + sawError);
    });
  }
);
