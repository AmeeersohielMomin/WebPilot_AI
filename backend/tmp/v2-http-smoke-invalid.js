    ...
    res.on('data', (chunk) => {
      ...
      while ((idx = buf.indexOf('\n\n')) !== -1) {
        ...
        if (evt.type === 'phase_diagnostic') {
          console.log('DIAG status=' + (evt.status || '') + ' attempt=' + (evt.attempt ?? '') + ' provider=' + (evt.provider || '') + ' model=' + (evt.model || '') + ' errorType=' + (evt.errorType || ''));
        }
        if (evt.type === 'plan') sawPlan = true;
        if (evt.type === 'module_complete') modules += 1;
        if (evt.type === 'file') files += 1;
...
function printSummary() {
  console.log('SUMMARY plan=' + sawPlan + ' modules=' + modules + ' files=' + files + ' complete=' + sawComplete + ' error=' + sawError + ' lastPhase=' + lastPhase + ' errorMsg=' + errorMsg);
}
